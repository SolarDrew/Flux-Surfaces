...
r = sys.argv[1]
tube_r = 'r{:02}'.format(int(r))

m = sys.argv[2]

#for tube_r in cfg.tube_radii:
#for r in range(12, 128, 6):
#for r in range(72, 128, 6):
#    tube_r = 'r{:02}'.format(r)
#for m in ['m0', 'm1', 'm2']:
print("Starting Tube {}, {}".format(tube_r, m), type(m))
print('../configure.py set SAC --usr_script={}'.format(m))
os.system('../configure.py set SAC --usr_script={}'.format(m))
cfg = SACConfig()
cfg.print_config()
#sys.exit()
gdf_files = glob.glob(os.path.join(cfg.gdf_dir, cfg.get_identifier()+'_0*.gdf'))
...
rank_indices = np.array_split(np.arange(180), size)
if not mpi:
...
for ring_height in range(5, 125, 10):  # in grid points
    """if os.path.exists(os.path.join(cfg.data_dir, tube_r, "{}_h{}_distance.npy".format(cfg.get_identifier(), ring_height))):
        print("Skipping height {}".format(ring_height))
        continue
    else:"""
    ...
    #import pdb; pdb.set_trace()
      #except IOError:
      #  print('Failed for {}, {}'.format(tube_r, n))
      #  continue
    ...
    try:
      if rank == 0:
        ...
        print('Could not save for {}, {}'.format(cfg.get_identifier(), ring_height))
        raise
